...
class SongRequest:
    ...
    def _validate_name(name: Optional[str]) -> Optional[str]:
        """Validate the optional requester name (spoken on air)"""
        if name is None:
            return None
        ...
        # The name gets read aloud and embedded in prompts — keep it tame
        allowed_chars = set(
            "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789 .,'-_"
        )
        for char in name:
            if char not in allowed_chars:
                raise ValidationError(f"Name contains invalid character: '{char}'")
