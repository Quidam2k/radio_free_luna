...
# Project root (two levels up from src/core/) — used to anchor relative
# defaults so behavior doesn't depend on the current working directory.
...
class Settings:
    ...
    def __init__(self):
        ...
        # Model used for both track analysis and DJ commentary.
        self.openai_model = os.getenv('OPENAI_MODEL', 'gpt-4o-mini')

        # Background worker that analyzes unanalyzed tracks and persists
        # results to the track_analysis table.
        self.auto_analyze_tracks = os.getenv('AUTO_ANALYZE_TRACKS', 'true').lower() == 'true'
        ...
        default_db_path = PROJECT_ROOT / 'data' / 'radio_free_luna.db'
        ...
        if self.database_url.startswith('sqlite:///'):
            ...
            db_file = Path(self.database_url[len('sqlite:///'):])
        ...
        # Save each broadcast as an MP3 in the archive directory
        self.archive_broadcasts = os.getenv('ARCHIVE_BROADCASTS', 'false').lower() == 'true'
        self.archive_directory = os.getenv('ARCHIVE_DIRECTORY', str(PROJECT_ROOT / 'archives'))
        ...
        self.log_file = os.getenv(
            'LOG_FILE',
            str(PROJECT_ROOT / 'logs' / 'radio_free_luna.log')
        )
