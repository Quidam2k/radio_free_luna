...
class RadioFreeLunaUI {
    ...
    async testVoice() {
        ...
        try {
            const result = await this.makeRequest(`${this.apiURL}/test-voice?text=${encodeURIComponent(text)}`, {
                method: 'POST'
            });
    ...
    async generateCommentary(type = 'contextual') {
        ...
        try {
            const commentary = await this.makeRequest(`${this.apiURL}/commentary?text_type=${type}`, {
                method: 'POST'
            });
