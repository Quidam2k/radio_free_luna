...
class TrackSequenceItem:
    ...
    requested_by: Optional[str] = None  # listener request — name spoken on air
...
class SessionManager:
    def __init__(self, database_url: str, openai_api_key: str, base_url: Optional[str] = None):
        ...
        self.sequencing_weights = {
            ...
            "discovery_factor": 0.15  # anti-repeat: real stations don't loop favorites
        }
    ...
    async def find_tracks_for_theme(self, theme: str, context: Dict) -> List[Dict]:
        ...
        """
        # Sync SQLAlchemy query — run off the event loop.
        return await asyncio.to_thread(self._find_tracks_for_theme_sync, theme, context)

    def _find_tracks_for_theme_sync(self, theme: str, context: Dict) -> List[Dict]:
        session = get_db()
        try:
    ...
    async def search_tracks(self, query: str, limit: int = 5) -> List[Dict]:
        """Search the library by title/artist for listener requests."""
    ...
    def pick_next_track(self, 
        ...
        for track in candidates:
            ...
            # Discovery factor (prefer less-played, not-recently-aired tracks)
            discovery_score = self.calculate_freshness(track)
    ...
    def calculate_freshness(self, track: Dict) -> float:
        """How 'fresh' a track is for rotation: 1.0 = ripe, 0.0 = burned out.

        Combines lifetime play count (capped) with a steep same-day repeat
        penalty so a track aired this morning won't reappear tonight.
        """
        score = 1.0

        play_count = track.get("play_count", 0) or 0
        score -= min(0.5, play_count / 50.0)

        last_played = track.get("last_played")
        if last_played:
            hours_ago = max(0.0, (datetime.utcnow() - last_played).total_seconds() / 3600.0)
            if hours_ago < 24:
                score -= 0.5 * (1.0 - hours_ago / 24.0)
    ...
    async def store_session(self, 
        ...
        """Store session in database"""
        # Sync SQLAlchemy writes — run off the event loop.
        await asyncio.to_thread(
            self._store_session_sync, session_id, theme, duration_minutes,
            tracks, commentary, context, parameters
        )

    def _store_session_sync(self,
                            session_id: str,
                            theme: str,
                            duration_minutes: int,
                            tracks: List[TrackSequenceItem],
