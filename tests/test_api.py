...
class TestHealthEndpoints:
    ...
    def test_root_endpoint(self, client):
        ...
        data = response.json()
        assert data["message"] == "Radio Free Luna - AI DJ System"
...
class TestSessionAPI:
    ...
    def test_create_session(self, mock_get_context, mock_create_session, client, sample_context):
        ...
        response = client.post(
            "/api/sessions",
            params={"theme": "rainy_day", "duration_minutes": 60}
        )
        assert response.status_code == 200
...
class TestVoiceAPI:
    ...
    def test_voice_synthesis(self, mock_synthesize, client):
        ...
        response = client.post(
            "/api/test-voice",
            params={"text": "Hello from Radio Free Luna"}
        )
        assert response.status_code == 200
